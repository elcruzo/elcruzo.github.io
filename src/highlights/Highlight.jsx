...
const TwoColumnGrid = () => {
  ...
  useLayoutEffect(() => {
    const calculateRects = () => {
      ...
      const rect = gridRef.current?.children
        ? Array.from(gridRef.current.children).map(child => child.getBoundingClientRect())
        : [];
      setCellsRect(rect);
    };
...
useLayoutEffect(() => {
  listRefs.forEach(ref => {
    if (ref.current) {
      ...
      setShowReadMoreButton(showReadMoreButton => showReadMoreButton || isContentOverflow);
    }
  });
}, [isOpenArrays]);

const renderListItem = (item, cellIndex, listItemIndex) => (
  <>
    <li>
      <span ref={listRefs[cellIndex]} style={isOpenArrays[cellIndex]?.[listItemIndex] ? null : listStyle}>
        {renderTextContent(item[`list${listItemIndex + 1}`])}
      </span>
    </li>
    {showReadMoreButton && (
...
const createLinesAndPaths = (cellsRect, gridRect) => {
  const lines = [];
  const paths = [];

  cellsRect.forEach((cell, index) => {
    if (index % 2 === 0 && index > 0) {
      const startY = cell.top + (cellsRect[index - 1].bottom - cell.top) / 2 - gridRect.top;
      const endX = cell.left - gridRect.left;

      lines.push(createLine(startY, endX));

      if (index < cellsRect.length - 2) {
        const midY = cellsRect[index - 1].bottom + (cellsRect[index + 1].top - cellsRect[index - 1].bottom) / 2 - gridRect.top;
        paths.push(createPath(startY, midY, endX));
      }
    }
  });
...
const { lines, paths } = createLinesAndPaths(cellsRect, gridRect);
...
return (
  <div ref={gridRef} style={gridContainerStyle} className="container">
    ...
    {jsonData.map((item, cellIndex) => (
      <div key={cellIndex} style={{ ...cellStyle, gridRow: cellIndex === 0 ? 'span 1' : 'span 2', marginTop: cellIndex === 1 ? secondCellMarginTop : '' }} className="mb-4">
        <h2 style={headerStyle}>{item.header}</h2>
        <p>{item.date}</p>
        <ul className='bulletpoints'>
          {item.list1 && renderListItem(item, cellIndex, 0)}
          {item.list2 && renderListItem(item, cellIndex, 1)}
          {item.list3 && renderListItem(item, cellIndex, 2)}
        </ul>
      </div>
    ))}
  </div>
