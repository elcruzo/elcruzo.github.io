...
//   const [isOpenArray, setIsOpenArray] = useState(Array(jsonData.length).fill(false));
 ...
 useLayoutEffect(() => {
  listRefs.forEach((listRef, index) => {
    if (listRef.current) {
      console.log(`scrollHeight of li[${index}]:`, listRef.current.scrollHeight);
      console.log(`clientHeight of li[${index}]:`, listRef.current.clientHeight);
      // Update the isOpenArrays state based on the heights of the <li> elements
      // You can use the same logic as before to determine whether to show the "Read More" button
    }
  });
}, [listRefs]);
